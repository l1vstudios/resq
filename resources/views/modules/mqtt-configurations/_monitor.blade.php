...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="d-flex align-items-center gap-2">
        <span class="badge" id="mqtt-gateway-badge">...</span>
        <button type="button" class="btn btn-success btn-sm" id="mqtt-gateway-start" title="Start MQTT Gateway">
            <i class="bx bx-play me-1"></i>Start
        </button>
        <button type="button" class="btn btn-warning btn-sm" id="mqtt-gateway-restart" title="Restart MQTT Gateway">
            <i class="bx bx-refresh me-1"></i>Restart
        </button>
        <button type="button" class="btn btn-danger btn-sm" id="mqtt-gateway-stop" title="Stop MQTT Gateway">
        ...
        @forelse($mqttConfigurations as $config)
            <tr data-mqtt-row="{{ $config->id }}">
                <td>
                    ...
                    @if(!empty($config->is_current))
                        <span class="badge bg-success-subtle text-success border border-success-subtle ms-1">Current</span>
                    @endif
                    <br>
                    <code>{{ $config->configuration_code }}</code>
                    <br>
                    <span class="small text-muted">{{ $config->broker_url }}</span>
                    @if(!empty($config->current_logger_codes))
                        <br>
                        <span class="small text-muted">Logger: {{ implode(', ', $config->current_logger_codes) }}</span>
                    @endif
                </td>
