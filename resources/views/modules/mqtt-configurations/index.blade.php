        ...
        <div class="card">
            <div class="card-body">
                ...
                <form method="POST" action="{{ route('mqtt-configurations.store') }}" id="mqtt-configuration-form">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Project</label><select name="project_id" class="form-select" required>@foreach($projects as $project)<option value="{{ $project->id }}">{{ $project->project_code }} - {{ $project->name }}</option>@endforeach</select></div>
                        <div class="col-md-6 mb-3"><label class="form-label">Configuration Code</label><input name="configuration_code" class="form-control" placeholder="MQTT-PROJECT-01" required></div>
                    </div>
                    <div class="mb-3"><label class="form-label">Name</label><input name="name" class="form-control" required></div>
                    <div class="mb-3"><label class="form-label">Broker URL</label><input name="broker_url" class="form-control" placeholder="mqtts://broker.example.com:8883" required></div>
                    <div class="row">
                        <div class="col-md-6 mb-3"><label class="form-label">Username</label><input name="username" class="form-control"></div>
                    ...
                    <div class="border rounded p-3 mb-3">
                        <div class="form-check form-switch mb-3"><input type="hidden" name="consumer_enabled" value="0"><input class="form-check-input" type="checkbox" name="consumer_enabled" value="1" id="mqtt-consumer-enabled"><label class="form-check-label fw-bold" for="mqtt-consumer-enabled">Consumer</label></div>
                        <div class="row">
                            <div class="col-md-9 mb-3"><label class="form-label">Input Topic</label><input name="consumer_topic" class="form-control" placeholder="project/telemetry/#"></div>
                            <div class="col-md-3 mb-3"><label class="form-label">QoS</label><select name="consumer_qos" class="form-select"><option>0</option><option>1</option><option>2</option></select></div>
                        </div>
                        <div class="mb-3"><label class="form-label">Example Output (JSON)</label><textarea name="example_payload" rows="5" class="form-control font-monospace" placeholder='{"sensor_code":"SNS-01","data":{"temperature":28.5}}'></textarea></div>
                        <div><label class="form-label">Sensor Code JSON Path</label><input name="sensor_code_path" class="form-control" value="sensor_code" placeholder="device.sensor_code"></div>
                    </div>

                    <div class="border rounded p-3 mb-3">
                        <div class="form-check form-switch mb-3"><input type="hidden" name="producer_enabled" value="0"><input class="form-check-input" type="checkbox" name="producer_enabled" value="1" id="mqtt-producer-enabled"><label class="form-check-label fw-bold" for="mqtt-producer-enabled">Producer</label></div>
                        <div class="row">
                            <div class="col-md-8 mb-3"><label class="form-label">Output Topic</label><input name="producer_topic" class="form-control" placeholder="project/output"></div>
                            <div class="col-md-2 mb-3"><label class="form-label">QoS</label><select name="producer_qos" class="form-select"><option>0</option><option>1</option><option>2</option></select></div>
                            <div class="col-md-2 mb-3"><label class="form-label">Retain</label><div class="form-check mt-2"><input type="hidden" name="producer_retain" value="0"><input type="checkbox" name="producer_retain" value="1" class="form-check-input"></div></div>
                        </div>
                        <div class="row">
                            <div class="col-md-6"><div class="form-check mb-2"><input type="hidden" name="publish_canonical" value="0"><input class="form-check-input" type="checkbox" name="publish_canonical" value="1" id="publish-canonical"><label class="form-check-label" for="publish-canonical">Publish canonical</label></div></div>
                            <div class="col-md-6"><div class="form-check mb-2"><input type="hidden" name="publish_warning" value="0"><input class="form-check-input" type="checkbox" name="publish_warning" value="1" id="publish-warning"><label class="form-check-label" for="publish-warning">Publish warning transition</label></div></div>
                        </div>
                        <div class="mb-3"><label class="form-label">Canonical Parameter Filter</label><select name="canonical_parameter_ids[]" class="form-select" multiple>@foreach(\App\Models\CanonicalParameter::orderBy('field_identity')->get() as $parameter)<option value="{{ $parameter->id }}">{{ $parameter->field_identity }}</option>@endforeach</select><small class="text-muted">Kosong berarti semua parameter.</small></div>
                        <div class="mb-3"><label class="form-label">Warning Level Filter</label><select name="warning_levels[]" class="form-select" multiple>@foreach(['Normal','Waspada','Siaga','Awas'] as $level)<option value="{{ $level }}">{{ $level }}</option>@endforeach</select></div>
                    ...
                    <div class="form-check form-switch mb-3"><input type="hidden" name="is_active" value="0"><input class="form-check-input" type="checkbox" name="is_active" value="1" id="mqtt-active" checked><label class="form-check-label" for="mqtt-active">Active / auto-connect</label></div>
