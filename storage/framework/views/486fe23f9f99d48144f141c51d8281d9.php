...
<?php
    ...
    $serialConfigs = $connectivity
        ->filter(fn ($item) => ($item['protocol'] ?? null) === 'Modbus RTU' || ! empty($item['serial_port']))
        ->values();
    $rednodeSerialConfig = $serialConfigs->first()
        ?? [];
    $rednodeLoggerCode = ($rednodeSerialConfig['logger_id'] ?? null)
        ?: ($dataLoggers->first()['id'] ?? '');
            ...
            <div class="col-12 p-3 border-bottom">
                <div class="modbus-panel p-0 mb-3">
                    <ul class="nav nav-tabs nav-justified px-2 pt-2" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link fw-bold" data-bs-toggle="tab" data-bs-target="#integration-rednode" type="button" role="tab">Gateway</button>
                        </li>
                    </ul>
...
<script>
    (function () {
        ...
        const rednodeRemoteHost = el('rednode-remote-host');
        ...
        function filterRednodeSensorsForLogger() {
            ...
            const remoteHost = selectedOption?.dataset.remoteHost || '';
            ...
            if (rednodeRemoteHost) {
                rednodeRemoteHost.value = remoteHost;
            }
        ...
        async function refreshRednodeStatus() {
            ...
            const data = await response.json().catch(() => ({}));
            if (!response.ok || data.ok === false) {
                throw new Error(data.message || 'Status gateway gagal dibaca.');
            }

            const statusEl = el('rednode-live-status');
            const online = Boolean(data.online);
            if (lastRednodeOnline !== online) {
                appendIntegrationLog('Gateway', online ? 'Kabar koneksi diterima dari gateway.' : 'Kabar koneksi gateway belum diterima atau sudah terlalu lama.', online ? 'Online' : 'Offline');
        ...
        async function submitRednodeConfig(showSuccess = true) {
            ...
            try {
                ...
                if (!response.ok || data.ok === false) {
                    ...
                    throw new Error(errors || data.message || 'Konfigurasi gateway gagal disimpan.');
                }
                ...
                const message = 'Berhasil mengganti sensor gateway ke ' + target + '. Gateway akan mengambil config terbaru otomatis.';
                el('rednode-live-port').textContent = portLabel || '-';
                if (showSuccess) {
                    appendIntegrationLog('Gateway', message, 'Sukses');
                    showMessage(message, 'success');
                }
                refreshRednodeStatus().catch(() => {});
                return data;
            } catch (error) {
                appendIntegrationLog('Gateway', error.message, 'Gagal');
        ...
        async function saveRednodeSerial(event) {
            ...
            showMessage('Config tersimpan. Menjalankan gateway di Data Logger...', 'info');
        ...
        async function controlRednode(action, options = {}) {
            ...
            try {
                ...
                if (!response.ok || data.ok === false) {
                    const errors = data.errors ? Object.values(data.errors).flat().join(' ') : '';
                    throw new Error(errors || data.message || 'Perintah gateway gagal.');
                }
                ...
                appendIntegrationLog('Gateway', data.message + output, 'Sukses');
